...
import { pageViews, dailyStats } from "@/drizzle/schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, duration, sessionId, path, referrer, userAgent } = body;

    // Si on reçoit un id et une durée, c'est une mise à jour du temps passé
    ...
    // 1. Insère la page view et récupère son ID
    ...
    return NextResponse.json({ ok: true, id: newPageView.id }, { status: 200 });
  } catch (error) {
    console.error("[Analytics/pageview]", error);
    // Retourne 200 quand même pour ne pas bloquer le client
