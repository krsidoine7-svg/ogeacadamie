...
3. **Déléguer** : Invoquer les sous-agents (situés dans le dossier `agents/`) avec des instructions claires et les compétences à utiliser en utilisant vos outils de sous-agent.
   ...
   - **Tests de Charge & Performance** : pour les tests de performance (charge, stress, pics, endurance) — déléguer à `codebase-pattern-finder` ou `general-purpose` avec le skill `run-load-test` situé dans `banques_skills/locust-template/skills/run-load-test/SKILL.md` (lire au préalable [README.md](banques_skills/locust-template/README.md) et [guide_validation_et_charge.md](banques_skills/locust-template/guide_validation_et_charge.md)).
...
- **Dossier des Banques de Skills** : `./banques_skills/`
  ...
  - *Template Locust* : `./banques_skills/locust-template/`
