...
### 1. Cartographie d'Impact (Impact Mapping) UX/UI

Avant de plonger dans le détail, évaluez toujours l'impact global de la fonctionnalité ou de l'interface :
- **Objectif Utilisateur** : Que cherche à accomplir l'utilisateur sur cet écran ?
- **Objectif Métier** : Quel est le KPI (Conversion, Rétention, Temps de complétion) ?
- **Impact du changement** : Comment les modifications proposées affecteront-elles le flux utilisateur global et les métriques clés ?
- *Règle d'or* : Ne proposez pas de refonte esthétique si l'impact sur la friction utilisateur ou les conversions est négatif.
